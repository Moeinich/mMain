...
public class DoWillowShortbow extends Task {
    int timer = 0;
    ...
    public void execute() {
        if (Inventory.stream().id(ItemList.WILLOW_LOGS_1519).count() == 0) {
            ...
            if (Inventory.stream().name("Willow logs").count() == 0) {
                Bank.depositAllExcept("Knife");
            }
            if (Inventory.stream().name("Knife").count() == 0) {
                Bank.withdraw("Knife", 1);
            }
            if (Bank.stream().name("Willow logs").first().stackSize() < 27) {
            ...
            while (!ScriptManager.INSTANCE.isStopping()) {
                ...
                if (currentCount >= initialCount) {
                    ...
                    if (timer >= 2) {
                        ...
                        if (Inventory.stream().id(ItemList.WILLOW_LOGS_1519).count() >= 1 && Game.tab(Game.Tab.INVENTORY)) {
